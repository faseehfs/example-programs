...
def FindResult(NumbersList, OperantsList):
    ...
    if len(NumbersList) > 2 and len(OperantsList) < 1:
        Result = (
            "Error! Your input contains more than two numbers and no operation symbol"
        )
        return Result
    if len(NumbersList) < 2 and len(OperantsList) > 1:
    ...
    if len(NumbersList) < 2 and len(OperantsList) < 1:
        Result = (
            "Error! Your input contains less than two numbers and no operation symbol"
        )
        return Result
...
while True:
    ...
    Result = GetRidOfDotZero(
        Result
    )  # This function also makes the floating point number a string.
    print("Result: ", Result)
